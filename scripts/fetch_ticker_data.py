...
base_path = os.path.dirname(os.path.dirname(os.path.realpath(__file__))) #change the file name here
...
tickers = ticker_list['Ticker'].to_numpy()


def download_data(ticker):
	data_manager = DataManager(api="alpha_vantage")
	imported_data = data_manager.get_historical_data(symbol=ticker,
                                                         interval='daily',
                                                         save_fetched=True)
